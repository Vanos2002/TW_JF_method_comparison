...
void compareEvolutionMethodsPhiWithQLT(
    ...
    AdaptiveRK4Integrator integrator(tolerance, 1e-10, 1e-2, 600000);
    ...
    outfile << std::scientific << std::setprecision(10);
    outfile << "phi,p_qlt,alpha_qlt,beta_qlt,p_fereisl,alpha_fereisl,beta_fereisl,"
            << "p_tw,alpha_tw,beta_tw,"
            << "qlt_vs_fereisl_max,qlt_vs_tw_max,fereisl_vs_tw_max\n";
    outfile.flush();
    ...
    std::cout << "[progress] Starting Fereisl integration..." << std::endl;
    auto rhs_fereisl = [max_PN_order](const BinaryState& s, const PhysicalParams& p, double /*phi*/) {
        return compute_secular_RHS_phi(s, p, max_PN_order);
    };
    auto result_fereisl = integrator.integrate(initial_state, params, rhs_fereisl, phi_start, phi_end);
    now = Clock::now();
    std::cout << "[progress] Fereisl integration complete in "
              << std::chrono::duration_cast<std::chrono::seconds>(now - section_start).count()
              << " s" << std::endl;
    section_start = now;
    ...
    for (int i = 0; i <= dense_samples; ++i) {
        ...
        auto fer = interpolateStateAtX(result_fereisl, phi);
    ...
    std::cout << "=== PHI-BASED COMPARISON (QLT vs Fereisl vs Tucker-Will) ===" << std::endl;
...
void plotLogPhiVsLogEpsilon(const std::vector<EpsPhiRow>& rows, const std::string& output_image) {
    ...
    fprintf(gp, "set grid\\n");
    fprintf(gp, "plot '-' with linespoints title 'QLT', '-' with linespoints title 'Fereisl', '-' with linespoints title 'TW'\\n");
    ...
    for (const auto& r : rows) {
        if (r.eps > 0.0 && r.phi_fereisl > 0.0 && std::isfinite(r.phi_fereisl)) {
            fprintf(gp, "%.16e %.16e\\n", std::log10(r.eps), std::log10(r.phi_fereisl));
        }
    }
    fprintf(gp, "e\\n");
