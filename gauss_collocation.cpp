...
public:
    ...
    std::pair<BinaryState, double> adaptiveStep(
        ...
        BinaryState y_full = gaussStep(state, params, rhs_func, x, h);
        BinaryState y_half_1 = gaussStep(state, params, rhs_func, x, h / 2.0);
        BinaryState y_half_2 = gaussStep(y_half_1, params, rhs_func, x + 0.5 * h, h / 2.0);
...
private:
    BinaryState gaussStep(
        ...
        std::array<SecularRHS, 2> K{{rhs_func(state, params, x), rhs_func(state, params, x + h)}};
        ...
        for (int iter = 0; iter < max_iter; ++iter) {
            ...
            std::array<SecularRHS, 2> K_new{{
                rhs_func(stage_states[0], params, x + c1 * h),
                rhs_func(stage_states[1], params, x + c2 * h)
            }};
...
SecularRHS compute_QLT_RHS_phi(const BinaryState& state,
    ...
    double c_val = (params_phi.eps != 0.0) ? 1.0 / params_phi.eps : std::numeric_limits<double>::infinity();
...
std::vector<EpsPhiRow> computeLogPhiVsLogEpsilonData(
    ...
    const double scan_max_step = 1e-2;
    const int scan_max_steps = 100000;
    AdaptiveGaussCollocationIntegrator integrator(tolerance, scan_min_step, scan_max_step, scan_max_steps);
...
void plotLogPhiVsLogEpsilon(const std::vector<EpsPhiRow>& rows, const std::string& output_image) {
    ...
    auto plot_with_python = [&](const std::string& path) -> bool {
        const char* py_cmd =
            ...
            "rows=[tuple(map(float,l.split())) for l in sys.stdin if l.strip()];"
            "out=sys.argv[1];"
            "xq=[math.log10(r[0]) for r in rows if r[0]>0 and r[1]>0 and math.isfinite(r[1])];"
            "yq=[math.log10(r[1]) for r in rows if r[0]>0 and r[1]>0 and math.isfinite(r[1])];"
            "xf=[math.log10(r[0]) for r in rows if r[0]>0 and r[2]>0 and math.isfinite(r[2])];"
            "yf=[math.log10(r[2]) for r in rows if r[0]>0 and r[2]>0 and math.isfinite(r[2])];"
            "xt=[math.log10(r[0]) for r in rows if r[0]>0 and r[3]>0 and math.isfinite(r[3])];"
    ...
    if (!plot_with_python(out_path)) {
        std::cout << "matplotlib plotting failed; skipping log-log plot generation." << std::endl;
        return;
    }
...
int main() {
    ...
    plotRHSFractionalDifferences(rhs_frac_rows, "rhs_fractional_differences_fromcpp.png");

    double eps_scan_phi_end = 100.0;
    std::cout << "Epsilon scan phi_end: " << eps_scan_phi_end << std::endl;
    auto eps_phi_rows = computeLogPhiVsLogEpsilonData(
        initial_state,
        params,
        max_PN_order,
        eps_scan_values,
