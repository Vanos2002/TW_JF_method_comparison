...
# THIS FILE COMPARES THE Feireisl-TUCKERWILL 4.5PN EXPRESSIONS TO AN
...
class AdaptiveGaussCollocationIntegrator:
    def __init__(self, tolerance: float = 1e-4,
                 ...
                 max_step: float = 0.1,
                 max_steps: int = 100,
                 stage_tolerance: float = 1e-12,
                 max_stage_iterations: int = 20):
        self.tolerance = tolerance
        self.min_step  = min_step
        self.max_step  = max_step
        self.max_steps = max_steps
        self.stage_tolerance = stage_tolerance
        self.max_stage_iterations = max_stage_iterations

        sqrt3 = math.sqrt(3.0)
        self.c1 = 0.5 - sqrt3 / 6.0
        self.c2 = 0.5 + sqrt3 / 6.0
        self.a11 = 0.25
        self.a12 = 0.25 - sqrt3 / 6.0
        self.a21 = 0.25 + sqrt3 / 6.0
        self.a22 = 0.25

    # -- low-level state helpers --------------------------------------------

    @staticmethod
    def _state_from_components(values: Tuple[float, float, float]) -> BinaryState:
        return BinaryState(p=values[0], alpha=values[1], beta=values[2])

    @staticmethod
    def _state_add_scaled(state: BinaryState,
                          k1: SecularRHS, scale1: float,
                          k2: SecularRHS, scale2: float) -> BinaryState:
        return BinaryState(
            p=state.p + scale1 * k1[0] + scale2 * k2[0],
            alpha=state.alpha + scale1 * k1[1] + scale2 * k2[1],
            beta=state.beta + scale1 * k1[2] + scale2 * k2[2],
        )

    @staticmethod
    def _max_relative_difference(lhs: SecularRHS, rhs: SecularRHS) -> float:
        max_rel_error = 0.0
        for left_value, right_value in zip(lhs, rhs):
            denom = max(abs(right_value), 1e-15)
            max_rel_error = max(max_rel_error, abs(left_value - right_value) / denom)
        return max_rel_error

    # -- low-level implicit collocation step --------------------------------

    def _gauss_legendre_step(self, state: BinaryState, params: PhysicalParams,
                             rhs_func: RHSFunc, x: float, h: float) -> BinaryState:
        k1 = rhs_func(state, params, x)
        k2 = list(k1)

        for _ in range(self.max_stage_iterations):
            stage1 = self._state_add_scaled(state, k1, h * self.a11, k2, h * self.a12)
            stage2 = self._state_add_scaled(state, k1, h * self.a21, k2, h * self.a22)

            next_k1 = rhs_func(stage1, params, x + self.c1 * h)
            next_k2 = rhs_func(stage2, params, x + self.c2 * h)

            delta = max(
                self._max_relative_difference(k1, next_k1),
                self._max_relative_difference(k2, next_k2),
            )
            k1, k2 = next_k1, next_k2
            if delta < self.stage_tolerance:
                break
        else:
            raise ValueError("Gauss collocation stage iteration failed to converge")

        return BinaryState(
            p=state.p + 0.5 * h * (k1[0] + k2[0]),
            alpha=state.alpha + 0.5 * h * (k1[1] + k2[1]),
            beta=state.beta + 0.5 * h * (k1[2] + k2[2]),
        )
    ...
    def adaptive_step(self, state: BinaryState, params: PhysicalParams,
        ...
        y1 = self._gauss_legendre_step(state, params, rhs_func, x, h)
        y_half = self._gauss_legendre_step(state, params, rhs_func, x, h / 2.0)
        y2 = self._gauss_legendre_step(y_half, params, rhs_func, x + 0.5 * h, h / 2.0)
...
def compute_log_phi_vs_log_epsilon_data(
    ...
    integrator = AdaptiveGaussCollocationIntegrator(
        tolerance,
        scan_min_step,
        scan_max_step,
        scan_max_steps,
...
def main() -> None:
    ...
    print("=== Adaptive Gauss Collocation Integration Comparison ===")
    ...
    plot_rhs_fractional_differences(rhs_frac_rows, "gauss_rhs_fractional_differences.png")
